...
@interface HZActivity()

@property (strong, nonatomic) UIView        *backGroundView;
@property (strong, nonatomic) NSString      *actionTitle;
@property (assign, nonatomic) NSInteger     postionIndexNumber;
@property (assign, nonatomic) BOOL          isHadTitle;
@property (assign, nonatomic) BOOL          isHadShareButton;
@property (assign, nonatomic) BOOL          isHadCancelButton;
@property (assign, nonatomic) CGFloat       HZActivityHeight;
@property (assign, nonatomic) id<HZActivityDelegate>delegate;

@end

@implementation HZActivity
        ...
        shareButtonImagesName:(NSArray *)shareButtonImagesName
{
    ...
    if (self) {
        ...
        [self creatButtonsWithTitle:title
                  ...
                  shareButtonTitles:shareButtonTitles
          withShareButtonImagesName:shareButtonImagesName];
...
- (void)show
{
    ...
    [UIView animateWithDuration:ANIMATE_DURATION animations:^{
        self.alpha = 1.0;
        [self.backGroundView setFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height-self.HZActivityHeight, [UIScreen mainScreen].bounds.size.width, self.HZActivityHeight)];
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle shareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray
{
    ...
    if (title) {
        ...
        self.HZActivityHeight = self.HZActivityHeight + 2*TITLE_INTERVAL_HEIGHT+TITLE_HEIGHT;
        ...
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                if (self.isHadTitle == YES) {
                    [shareButton setFrame:CGRectMake(SHAREBUTTON_INTERVAL_WIDTH+((line-1)*(SHAREBUTTON_INTERVAL_WIDTH+SHAREBUTTON_WIDTH)), self.HZActivityHeight+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)), SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                }
                else{
    ...
    if (shareButtonTitlesArray) {
        if (shareButtonTitlesArray.count > 0 && shareButtonImagesNameArray.count > 0) {
            for (int j = 1; j < shareButtonTitlesArray.count+1; j++) {
                ...
                //有Title的时候
                if (self.isHadTitle == YES) {
                    [shareLabel setFrame:CGRectMake(SHARETITLE_INTERVAL_WIDTH+((line-1)*(SHARETITLE_INTERVAL_WIDTH+SHARETITLE_WIDTH)), self.HZActivityHeight+SHAREBUTTON_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)), SHARETITLE_WIDTH, SHARETITLE_HEIGHT)];
                }
    ...
    if (shareButtonImagesNameArray && shareButtonImagesNameArray.count > 0) {
        ...
        if (self.isHadTitle  == YES) {
            self.HZActivityHeight = self.HZActivityHeight + totalColumns*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT);
        }
        else{
            self.HZActivityHeight = SHAREBUTTON_INTERVAL_HEIGHT + totalColumns*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT);
        }
    }
    ...
    if (cancelButtonTitle) {
        ...
        if (self.isHadTitle == NO && self.isHadShareButton == NO) {
            self.HZActivityHeight = self.HZActivityHeight + cancelButton.frame.size.height+(2*BUTTON_INTERVAL_HEIGHT);
        }
        //当有title或destructionButton或otherbuttons时
        if (self.isHadTitle == YES || self.isHadShareButton == YES) {
            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.HZActivityHeight, cancelButton.frame.size.width, cancelButton.frame.size.height)];
            self.HZActivityHeight = self.HZActivityHeight + cancelButton.frame.size.height+BUTTON_INTERVAL_HEIGHT;
        }
