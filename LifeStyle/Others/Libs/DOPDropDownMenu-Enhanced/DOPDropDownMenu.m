...
#define kTableViewHeight 260
#define kButtomImageViewHeight 21

#define kTextColor [UIColor colorWithRed:0.51/255.0 green:0.51/255.0 blue:0.51/255.0 alpha:1]
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        self.layer.backgroundColor = kCollectionHeaderColor.CGColor;
...
//        _buttomImageView.image = [UIImage imageNamed:@"drop_chose_bottom"];
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    //hzf
    layer.backgroundColor = kCollectionHeaderColor.CGColor;
    return layer;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _leftTableView) {
        ...
        if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:_currentSelectedMenudIndex]> 0){
            cell.accessoryView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"drop_chose_arrow_nor"] highlightedImage:[UIImage imageNamed:@"drop_chose_arrow_sel"]];
        } else {
